<x-dashboardlayout name="{{ $name }}" title="Add Competition Result">
    <div class="mb-8 space-y-6">
        <!-- Header Section -->
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold tracking-tight" style="font-family: var(--font-header1); color: var(--color-white);">Add Result for {{ $competition->name }}</h1>
                <p class="text-sm mt-1" style="color: rgba(255,255,255,0.6); font-family: var(--font-body1);">Record a beneficiary's achievement or placement in this competition</p>
        ...
        <form method="POST" action="{{ route('competition-results.store', $competition) }}" class="space-y-6">
            @csrf

            @include('competition_results._form')
