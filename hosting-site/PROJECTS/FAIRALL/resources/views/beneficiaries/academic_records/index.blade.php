<x-dashboardlayout name="{{ $name }}" title="Academic Records">
    <div class="mb-8 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-white" style="font-family: var(--font-header1);">Academic
                    Records</h1>
                <p class="text-white/60 text-sm mt-1" style="font-family: var(--font-body1);">Manage academic performance
                    and history</p>
        ...
        <div class="rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-xl"
            ...
            <div class="px-4 sm:px-6 py-3 sm:py-4 border-b"
                ...
                <div class="flex items-center justify-between gap-4 flex-wrap">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <h2 class="font-semibold text-white" style="font-family: var(--font-header1);">Academic
                                Records</h2>
                            <p class="text-xs text-white/50">{{ $beneficiary->display_name }}</p>
                        </div>
                    </div>
                    <a href="/beneficiaries/{{ $beneficiary->id }}/academic-records/create{{ $beneficiary->educationEnrollments->isNotEmpty() ? '?enrollment_id=' . $beneficiary->educationEnrollments->sortByDesc('academic_year_start_date')->first()->id : '' }}"
            ...
            <div class="px-4 sm:px-6 pb-3">
                <x-bulk-actions-bar bulkUrl="{{ route('beneficiaries.academic-records.bulk-destroy', $beneficiary) }}" tableId="academic-{{ $beneficiary->id }}" label="academic records" />
            </div>
            <div class="p-6">
                <div class="overflow-x-auto">
                    <table class="w-full min-w-225" data-bulk-table="academic-{{ $beneficiary->id }}">
                        <thead>
                            <tr style="border-bottom: 1px solid rgba(255,255,255,0.06);">
                                ...
                                <th class="text-center px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-basefont-semibold uppercase tracking-wider"
                                    style="color: rgba(255,255,255,0.5); font-family: var(--font-body1);">ID</th>
                                <th class="text-center px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-basefont-semibold uppercase tracking-wider"
                                ...
                                <th class="text-center px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-basefont-semibold uppercase tracking-wider"
                                    style="color: rgba(255,255,255,0.5); font-family: var(--font-body1);">Term</th>
                                <th class="text-center px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-basefont-semibold uppercase tracking-wider"
                                    style="color: rgba(255,255,255,0.5); font-family: var(--font-body1);">Grade Level
                                </th>
                                <th class="text-center px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-basefont-semibold uppercase tracking-wider"
                                    style="color: rgba(255,255,255,0.5); font-family: var(--font-body1);">GWA</th>
                                <th class="text-center px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-basefont-semibold uppercase tracking-wider"
                            ...
                            @forelse ($academicRecords as $index => $record)
                                <tr class="transition-colors hover:bg-white/5 {{ $index % 2 == 0 ? 'bg-white/5' : 'bg-transparent' }}"
                                    ...
                                    <td class="px-3 sm:px-4 py-3 align-middle text-center text-sm"
                                        ...
                                        {{ $record->id }}</td>
                                    <td class="px-4 py-3 text-center text-sm text-white">{{ $record->school_name }}</td>
                                    <td class="px-3 sm:px-4 py-3 align-middle text-center text-sm"
                                        style="color: rgba(255,255,255,0.8);">
                                        {{ $record->academic_year }}</td>
                                    <td class="px-3 sm:px-4 py-3 align-middle text-center text-sm"
                                        style="color: rgba(255,255,255,0.8);">
                                        {{ $record->term }}</td>
                                    <td class="px-3 sm:px-4 py-3 align-middle text-center text-sm"
                                        style="color: rgba(255,255,255,0.8);">
                                        {{ $record->grade_level }}</td>
                                    <td class="px-3 sm:px-4 py-3 align-middle text-center">
                                        @php
                                            $gwa = $record->gwa;
                                            if ($gwa >= 90) {
                                    ...
                                    <td class="px-3 sm:px-4 py-3 align-middle text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="/beneficiaries/{{ $beneficiary->id }}/academic-records/{{ $record->id }}"
                                            ...
                                            <a href="/beneficiaries/{{ $beneficiary->id }}/academic-records/{{ $record->id }}/edit"
                                                class="p-1.5 sm:p-2 rounded-lg transition-all hover:bg-white/10 hover:scale-110"
                                                title="Edit">
                                                <svg class="w-4 h-4"
                                                    style="color: rgba(255,255,255,0.5); font-family: var(--font-body1);"
                                            ...
                                            <button type="button"
                                                ...
                                                <svg class="w-4 h-4" style="color: var(--color-danger);"
                                                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2"
                                            ...
                                            <x-confirm-dialog id="deleteModal_{{ $record->id }}"
                                                ...
                                                message="Are you sure you want to delete the academic record for <strong>{{ $record->school_name }}</strong> ({{ $record->academic_year }})? This record will be moved to the archive and automatically deleted after 90 days."
                                                deleteUrl="/beneficiaries/{{ $beneficiary->id }}/academic-records/{{ $record->id }}" />
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="8" class="px-4 py-12 text-center">
                                        ...
                                        <p class="text-sm" style="color: rgba(255,255,255,0.4);">No academic records
                                            found.</p>
                                    </td>
                                </tr>
                            @endforelse
            ...
            @if (method_exists($academicRecords, 'hasPages') && $academicRecords->hasPages())
                <div class="flex justify-center pb-6">
                    {{ $academicRecords->links('pagination.custom') }}
                </div>
            @endif
        </div>
    </div>
