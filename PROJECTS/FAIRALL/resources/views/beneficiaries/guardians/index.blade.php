<x-dashboardlayout name="{{ $name }}" title="Guardians">
    <div class="mb-8 space-y-6">
        <!-- Header Section -->
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-white" style="font-family: var(--font-header1);">Guardians</h1>
                <p class="text-white/60 text-sm mt-1" style="font-family: var(--font-body1);">Manage guardian information for {{ $beneficiary->display_name }}</p>
            ...
            <div class="px-4 sm:px-6 py-3 sm:py-4 border-b"
                ...
                <div class="flex items-center justify-between gap-4 flex-wrap">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <h2 class="font-semibold text-white" style="font-family: var(--font-header1);">Guardian Records</h2>
                            <p class="text-xs text-white/50">Manage guardians and their information</p>
                        </div>
                    </div>
                    <a href="/beneficiaries/{{ $beneficiary->id }}/guardians/create" 
            ...
            <div class="p-6">
                @if ($guardians->isEmpty())
                    <div class="text-center py-12">
                        ...
                        <p class="text-sm" style="color: rgba(255,255,255,0.4);">No guardians found.</p>
                        <p class="text-xs mt-1" style="color: rgba(255,255,255,0.3);">Click "Add Guardian" to add a guardian for this beneficiary.</p>
                    </div>
                @else
                        ...
                        <table class="w-full min-w-225">
                            <thead>
                                <tr style="border-bottom: 1px solid rgba(255,255,255,0.06);">
                                    ...
                                    <th class="text-center px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-basefont-semibold uppercase tracking-wider" style="color: rgba(255,255,255,0.5); font-family: var(--font-body1);">Name</th>
                                    <th class="text-center px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-basefont-semibold uppercase tracking-wider" style="color: rgba(255,255,255,0.5); font-family: var(--font-body1);">Contact</th>
                                    <th class="text-center px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-basefont-semibold uppercase tracking-wider" style="color: rgba(255,255,255,0.5); font-family: var(--font-body1);">Occupation</th>
                                ...
                                @forelse ($guardians as $index => $guardian)
                                    <tr class="transition-colors hover:bg-white/5 {{ $index % 2 == 0 ? 'bg-white/5' : 'bg-transparent' }}" style="border-bottom: 1px solid rgba(255,255,255,0.04);">
                                        ...
                                        <td class="px-3 sm:px-4 py-3 align-middle text-center">
                                            <div class="text-sm text-white">{{ $guardian->first_name }} {{ $guardian->middle_name }} {{ $guardian->last_name }}</div>
                                        </div>
                                        <td class="px-3 sm:px-4 py-3 align-middle text-center">
                                            <div class="flex items-center justify-center gap-1">
                                                <span class="text-sm" style="color: rgba(255,255,255,0.7);">{{ $guardian->formatted_contact }}</span>
                                            </div>
                                        </div>
                                        <td class="px-3 sm:px-4 py-3 align-middle text-center text-sm" style="color: rgba(255,255,255,0.7);">{{ $guardian->job ?? 'N/A' }}</div>
                                        <td class="px-3 sm:px-4 py-3 align-middle text-center">
                                        ...
                                        <td class="px-3 sm:px-4 py-3 align-middle text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                <a href="/beneficiaries/{{ $beneficiary->id }}/guardians/{{ $guardian->id }}" 
                                                ...
                                                <button class="p-1.5 rounded-lg transition-all hover:bg-red-500/20" title="Delete"
                                                        onclick="document.getElementById('deleteModal_{{ $guardian->id }}').showModal()">
                                                ...
                                                <x-confirm-dialog id="deleteModal_{{ $guardian->id }}"
