<x-dashboardlayout name="{{ $name }}" title="Edit Home Visit">
    <div class="mb-8 space-y-6">
        <!-- Header Section -->
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold tracking-tight" style="font-family: var(--font-header1); color: var(--color-white);">Edit Home Visit</h1>
                <p class="text-sm mt-1" style="color: rgba(255,255,255,0.6); font-family: var(--font-body1);">Update home visit details</p>
        ...
        <form method="POST" action="{{ route('home-visits.update', $homeVisit) }}" class="space-y-6">
            @csrf
            @method('PUT')

            @include('home_visits._form')

            <!-- Form Actions -->
            <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4">
                <a href="{{ route('home-visits.show', $homeVisit) }}" 
