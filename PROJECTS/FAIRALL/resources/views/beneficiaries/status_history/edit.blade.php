<x-dashboardlayout name="{{ $name }}" title="Edit Status Record">
    <div class="mb-8 space-y-6">
        <!-- Header Section -->
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-white" style="font-family: var(--font-header1);">Edit Status Record</h1>
                <p class="text-white/60 text-sm mt-1" style="font-family: var(--font-body1);">Update status record for {{ $beneficiary->display_name }}</p>
                <div class="w-16 h-1 mt-2 rounded-full" style="background-color: var(--color-accent1);"></div>
            </div>
        </div>

        <form method="POST" action="/beneficiaries/{{ $beneficiary->id }}/status-history/{{ $record->id }}">
            @csrf
            @method('PUT')
                ...
                <div class="px-4 sm:px-6 py-3 sm:py-4 border-b" style="border-color: rgba(255,255,255,0.08); background: linear-gradient(135deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.02) 100%);">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-white/50">Edit the beneficiary status details</p>
                        </div>
                    </div>
                </div>
                <div class="p-6">
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select name="beneficiary_status_type_id" class="w-full px-4 py-2.5 rounded-lg border focus:outline-none focus:ring-2 focus:ring-accent1 transition-all appearance-none" 
                                    ...
                                    <option value="" disabled style="color: var(--color-primary1);">Select a status</option>
                                    @foreach ($statusTypes as $id => $label)
                                        <option value="{{ $id }}" @selected(old('beneficiary_status_type_id', $record->beneficiary_status_type_id) == $id) style="color: var(--color-primary1);">{{ $label }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <input type="date" name="start_date" class="w-full px-4 py-2.5 text-primary1 rounded-lg border focus:outline-none focus:ring-2 focus:ring-accent1 transition-all" 
                                   ...
                                   value="{{ old('start_date', optional($record->start_date)->format('Y-m-d')) }}" required>
                        ...
                        <div>
                            ...
                            <input type="date" name="end_date" class="w-full px-4 py-2.5 text-primary1 rounded-lg border focus:outline-none focus:ring-2 focus:ring-accent1 transition-all" 
                                   style="font-family: var(--font-body1); background-color: rgba(255,255,255,0.1); border-color: rgba(255,255,255,0.2);"
                                   value="{{ old('end_date', optional($record->end_date)->format('Y-m-d')) }}">
