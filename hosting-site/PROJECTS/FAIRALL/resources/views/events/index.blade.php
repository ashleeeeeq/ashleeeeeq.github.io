<x-dashboardlayout name="{{ $name }}" title="Events">
    <div class="mb-8 space-y-6">
        <!-- Header Section  -->
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold tracking-tight"
                    style="font-family: var(--font-header1); color: var(--color-white);">Events</h1>
                <p class="text-sm mt-1" style="color: rgba(255,255,255,0.6); font-family: var(--font-body1);">Schedule
                    events and track attendance records</p>
        ...
        <div class="flex gap-2 mb-6">
            <a href="{{ request()->fullUrlWithQuery(['filter' => 'upcoming', 'events_page' => 1]) }}"
            ...
            <a href="{{ request()->fullUrlWithQuery(['filter' => 'completed', 'events_page' => 1]) }}"
            ...
            <form method="GET" action="">
                ...
                <div class="relative max-w-md">
                    ...
                    <input type="text" name="search" placeholder="Search events..." value="{{ request('search') }}"
                        class="w-full rounded-xl pl-10 pr-4 py-2 text-sm"
        ...
        <div class="grid gap-4 sm:gap-5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div class="flex items-start justify-between gap-2 sm:gap-3">
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs mt-1 wrap-break-word"
                                    ...
                                    {{ $event->program?->program_name ?? 'No program' }}</p>
                            </div>
                            <span
                                class="inline-flex px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap shrink-0"
                                style="background-color: rgba(255,204,51,0.15); color: var(--color-accent1);">
                                {{ $event->eventType?->name ?? 'Event type' }}
                            </span>
                            <span
                                class="inline-flex px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap shrink-0"
                                style="background-color: {{ (!$event->end || $event->end >= now()) ? 'rgba(16,185,129,0.15)' : 'rgba(255,255,255,0.05)' }}; color: {{ (!$event->end || $event->end >= now()) ? 'var(--color-success)' : 'rgba(255,255,255,0.5)' }};">
                                {{ (!$event->end || $event->end >= now()) ? 'Upcoming' : 'Completed' }}
                            </span>
                        </div>
                        ...
                        <div class="space-y-2">
                            <div class="flex items-center gap-2 min-w-0">
                                ...
                                <span class="text-xs sm:text-sm wrap-break-word"
                                    style="color: rgba(255,255,255,0.7); font-family: var(--font-body1);">{{ $event->location ?: 'No location set' }}</span>
                            </div>
                            <div class="flex items-center gap-2 min-w-0">
                                <div class="w-5 h-5 sm:w-6 sm:h-6 rounded-lg flex items-center justify-center shrink-0"
                                ...
                                <span class="text-xs sm:text-sm wrap-break-word"
                                    style="color: rgba(255,255,255,0.7); font-family: var(--font-body1);">{{ $event->start?->format('M d, Y g:i A') }}</span>
                            </div>
                        </div>

                        <!-- Attendance & Creator Row -->
                        <div class="flex flex-col xs:flex-row xs:items-center justify-between gap-2 pt-3 border-t"
                            ...
                            <div class="flex items-center gap-1.5">
                                ...
                                <span class="text-xs sm:text-sm"
                                    style="color: rgba(255,255,255,0.6);">{{ $event->attendances_count }}
                                    record/s</span>
                            </div>
                            <div class="flex items-center gap-1.5">
                                <svg class="w-3.5 h-3.5 sm:w-4 sm:h-4" style="color: rgba(255,255,255,0.5);"
                                ...
                                <span class="text-xs sm:text-sm truncate max-w-30 xs:max-w-none"
                                    style="color: rgba(255,255,255,0.6);">{{ $event->creator?->display_name ?? 'Staff' }}</span>
                            </div>
                        </div>
            ...
            @empty
                <div class="col-span-full rounded-2xl p-8 sm:p-12 text-center"
                    ...
                    <p class="text-sm" style="color: rgba(255,255,255,0.4); font-family: var(--font-body1);">No events
                        found.</p>
                </div>
            @endforelse
        </div>
