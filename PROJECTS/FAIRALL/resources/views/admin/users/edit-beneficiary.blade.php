<x-dashboardlayout name="{{ $name }}" title="Edit Beneficiary Account">
    <div class="mb-8">
        <!-- Header Section -->
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                ...
                <p class="text-sm mt-1" style="color: rgba(255,255,255,0.6); font-family: var(--font-body1);">
                    Update the linked user account for this beneficiary
                </p>
        ...
        <div class="rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-xl mb-6" style="background-color: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.08);">
            <div class="px-4 sm:px-6 py-3 sm:py-4 border-b" style="border-color: rgba(255,255,255,0.08); background: linear-gradient(135deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.02) 100%);">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-semibold text-white text-sm sm:text-base" style="font-family: var(--font-header1);">Beneficiary Profile</h3>
                        <p class="text-xs" style="color: rgba(255,255,255,0.5);">Linked beneficiary information</p>
                    </div>
                </div>
            </div>
            <div class="p-4 sm:p-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                        ...
                        <p class="text-sm font-medium text-white mt-1" style="font-family: var(--font-body1);">{{ $beneficiary?->display_name ?? 'N/A' }}</p>
                    </div>
                    <div>
                        ...
                        <p class="text-sm text-white/70 mt-1" style="font-family: var(--font-body1);">{{ $beneficiary?->formatted_contact ?? 'N/A' }}</p>
                    </div>
                    <div class="sm:col-span-2 lg:col-span-1">
                        <p class="text-xs font-semibold uppercase tracking-wider" style="color: rgba(255,255,255,0.5); font-family: var(--font-body1);">Grade Level</p>
                        <p class="text-sm text-white/70 mt-1" style="font-family: var(--font-body1);">{{ $beneficiary?->grade_level ?? 'N/A' }}</p>
                    </div>
                    <div class="sm:col-span-2 lg:col-span-1">
                        <p class="text-xs font-semibold uppercase tracking-wider" style="color: rgba(255,255,255,0.5); font-family: var(--font-body1);">Programs</p>
                        <div class="flex flex-wrap gap-1 mt-1">
                            @php
                                $programs = $beneficiary?->programs ?? collect();
                            @endphp
                            @if ($programs->count() > 0)
                                @foreach ($programs as $program)
                                    <span class="inline-flex px-2 py-0.5 text-xs rounded-full whitespace-nowrap" style="background-color: rgba(255,204,51,0.15); color: var(--color-accent1); font-family: var(--font-body1);">
                                        {{ $program->program_name ?? 'N/A' }}
                                    </span>
                                @endforeach
                            @else
                                <span class="text-sm text-white/50" style="font-family: var(--font-body1);">N/A</span>
                            @endif
                        </div>
                    </div>
                </div>
        ...
        <form method="POST" action="/users/{{ $user->id }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="email" name="email" 
                                class="w-full px-4 py-2.5 text-primary1 rounded-xl border focus:outline-none focus:ring-2 focus:ring-accent1 focus:border-accent1 transition-all duration-200 hover:border-accent1"
                                style="font-family: var(--font-body1); background-color: rgba(255,255,255,0.05); border-color: rgba(255,255,255,0.15);"
                                value="{{ old('email', $user->email) }}" placeholder="beneficiary@example.com" required>
            ...
            <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4">
                <a href="/users?tab=beneficiaries" 
